...
BEGIN
	...
	-- SÓ FAZ O PROCESSAMENTO SE A DATA DE REFERÊNCIA DA JORNADA FOR MENOR OU IGUAL AO GETDATE() -- 04/11/2020 - JEAN PAUL - DEMANDA: 138
	if @datajornada <= getdate()
	BEGIN
		select @mes=cartamesbase,@ano=cartaanobase,@ctococodigo=ctococodigo from tbgabcartaodeponto (nolock) 
			...
			BEGIN
				...
				begin	
					...
					begin
						...
						begin
							...
							begin try
								...
								update tbgabcartaodeponto set cartaadn = @minutos where funcicodigo = @funcicodigo and cartadatajornada = @datajornada

								-- ATUALIZA O VALOR DA HORA DIA
								-- 1º set @m1 = (select minuto from dbo.retornarSomaHorasFuncionario(@funcicodigo,@datajornada))
								update tbgabcartaodeponto set cartacargahorariarealizada = @m1 where funcicodigo = @funcicodigo and cartadatajornada = @datajornada
							end try
							begin catch
					...
					begin
						...
						begin
							...
							begin try
								...
								update tbgabcartaodeponto set cartahorasextra = @minutos, cartacargahorariarealizada = @m1 where funcicodigo = @funcicodigo and cartadatajornada = @datajornada
							end try
							begin catch
								insert into tbgabduplicados (funcicodigo,datajornada,rotina_origem,error)
								values (@funcicodigo,@datajornada,@origem,ERROR_MESSAGE())
					...
					begin
						...
						begin
							...
							begin try
								...
								begin
									update tbgabcartaodeponto set cartahorasextra = @minutos where funcicodigo = @funcicodigo and cartadatajornada = @datajornada
								end
							end try
							begin catch
								insert into tbgabduplicados (funcicodigo,datajornada,rotina_origem,error)
					...
					begin
						...
						-- 2º set @minutos = (select horasfalta from dbo.retornarSomaHorasFuncionario(@funcicodigo,@datajornada))
			...
			begin
				...
				BEGIN
					...
					begin
						...
						begin
							set @minutos = (select minuto from dbo.retornarPeriodoFuncionario(@funcicodigo,@datajornada,5))
						...
						begin
							...
							begin
								...
								-- 3º set @m1 = (select minuto from dbo.retornarSomaHorasFuncionario(@funcicodigo,@datajornada))
								set @minutos = @minutos + @m1
								update tbgabcartaodeponto set cartacargahorariarealizada = @minutos where funcicodigo = @funcicodigo and cartadatajornada = @datajornada
							end
							-- AGREGA O VALOR JÁ EXISTENTE COM UM EVENTUAL NOVO TOTALIZADOR DE CATEGORIA HORA EXTRA OU CATEGORIA HORA REALIZADA
							set @minutos = (select valor from dbo.retornarTotalTotalizadoresDia(@funcicodigo, @datajornada))
							update tbgabcartaodeponto set cartahorasextra = @minutos where funcicodigo = @funcicodigo and cartadatajornada = @datajornada
		...
		-- ===========================================================
			...
			begin
				...
				select @complementar = btotal, @cred = cred, @deb = deb from dbo.retornarCredDeb(@funcicodigo,@datajornada)
				-- PEGA HORAS FALTA DO DIA CASO TENHA.
				select @horasfalta = @deb
				...
				/* 4º if ((select saida from dbo.retornarSomaHorasFuncionario(@funcicodigo,@datajornada)) = 'FALTA' or (select saida from dbo.retornarSomaHorasFuncionario(@funcicodigo,@datajornada)) = 'INCONSISTÊNCIA' ) and @ctococodigo <> 2
				begin 
				...
				BEGIN
					...
					begin
						update tbgabcartaodeponto set cartacreditobh = @cred, cartadebitobh = @deb where funcicodigo = @funcicodigo and cartadatajornada = @datajornada
						...
						begin 
							...
							update tbgabcartaodeponto set cartahorasextra = @he where funcicodigo = @funcicodigo and cartadatajornada = @datajornada
						end
