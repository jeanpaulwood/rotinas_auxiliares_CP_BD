...
BEGIN
		...
		declare dias cursor FAST_FORWARD FORWARD_ONLY READ_ONLY for
		with dateRange as
		(
		  select dt = @startDate
		...
		select 
		dt,
		DATEPART(dw,dt) as dia,
		convert(varchar(MAX),dt,103) from dateRange OPTION (MAXRECURSION 1000)

		open dias
			fetch next from dias into @dt,@dia,@dtformat
			...
			--	set @indicacao = (select indicacao from cjRetornarHorarioEscala (@cod_escala,@dt,@dt))     -- FOI OTIMIZADO NO SELECT DE RECUPERACAO DE HORARIO E INDICACAO
				...
				insert into @tabela values (@dtformat,@dia,@dt,@codigo_h,@indicacao,@cod_escala,@cod_cc,@feriado,@feriatipo,
